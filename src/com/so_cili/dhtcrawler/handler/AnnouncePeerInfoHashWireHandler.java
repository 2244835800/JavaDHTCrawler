...
public class AnnouncePeerInfoHashWireHandler implements IInfoHashHandler {
	...
	// Logger.getLogger(AnnouncePeerInfoHashWireHandler.class);
				...
				parseNext(protocolLen + 48, new NextFunction() {
					...
					public void onFunction(byte[] buff) throws IOException {
						...
						if (handshake[5] == 0x10) {
							...
							// logger.error("remote peer don't support download
							// metadata.");
							release();
						}
					}
	...
	private void onFinished() throws UnsupportedEncodingException {
		...
		System.out.println("==========================================");
		System.out.println("info_hash" + ByteUtil.byteArrayToHex(info_hash));
		System.out.println("bittorrent:" + NodeIdUtil.SHA1(metadata));
		System.out.println("==========================================");
		try {
			writeToFile();
		} catch (Exception e) {
			System.out.println("种子文件写入失败");
		}
		release();
	}
	...
	private String decode_utf8(String encoding, Map<String, Object> map, String key)
		...
		return new String((byte[]) map.get(key), Charset.forName("UTF-8"));
	}
	...
	private String decode_utf8_2(String encoding, Map<String, Object> map, String key)
		...
		return new String(((List<byte[]>) map.get(key)).get(0), Charset.forName("UTF-8"));
	}
	...
	private Torrent parseTorrent(Map<String, Object> map) throws UnsupportedEncodingException {
		...
		/*
		 * if (map.containsKey("creation date")) { torrent.setCreationDate(new
		 * Date(((BigInteger) map.get("creation date")).longValue())); } else {
		 ...
		 * torrent.setComment(decode_utf8(encoding, map,
		 * "comment").toString().substring(0, 300)); }
		 * 
	...
	private void writeToFile() throws IOException {
		String path = "D://" + NodeIdUtil.SHA1(metadata) + ".torrent";
